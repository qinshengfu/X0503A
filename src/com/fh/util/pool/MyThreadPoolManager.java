...
public class MyThreadPoolManager {
    ...
    /**
     * 功能描述： 要确保该类只有一个实例对象，避免产生过多对象消费资源，所以采用单例模式
     * @author Ajie
    ...
    /**
     * 功能描述：使用线程池，线程池中线程的创建完全是由线程池自己来维护的，我们不需要创建任何的线程
    ...
    public static void shutdown() {
        ...
        if (getsInstance().executor != null) {
            // shutdownNow首先将线程池的状态设置为STOP，然后尝试停止所有正在执行或暂停任务的线程，并返回等待执行任务的列表
            getsInstance().executor.shutdownNow();

            // shutdown只是将线程池的状态设置成SHUTDOWN状态，然后中断没有在执行的任务。
            // executor.shutdown();
        }
    }
