 ...
 */
public class EmailVerificaCodeUtil {
    //配置信息
    ...
    //邮件编辑信息(仅需写上自己的)
    private static final String MAIL_FROM ="xingyi-wei@foxmail.com";//邮件发送人
    private static final String MAIL_FROM_PASSWORD ="cghjwkjfmqnzbicg";//邮件发送人授权码
    private static final String MAIL_SUBJECT_REGISTER ="星辉国际【注册链接】";//邮件主题(注册链接)
    private static final String MAIL_SUBJECT_VERIFY_CODE ="星辉国际【验证码】";//邮件主题(邮箱验证码)
    private static final String MAIL_ORGANIZATION ="星辉国际";//邮件组织
    private static final String MAIL_WEBSITE ="www.xhgjvip.com";//组织主页
    private static final String MAIL_ORGANIZATION_LOGO ="http://ww1.sinaimg.cn/large/006cqHgvgy1ga95y1x2i9j30sg0sggmt.jpg";//网站logo
    private static final String MAIL_ORGANIZATION_QRCODE ="http://a4.qpic.cn/psb?/V110yGYF2cTov5/ZOjCd6UqH0Me*f03USUjD5cUtb9bBeTjasA5H5QjoM!/m/dL8AAAAAAAAAnull&bo=CQM1AwAAAAARBww!&rf=photolist&t=5";//网站二维码图片


    /**
     * 功能描述：邮箱发送器，验证码通过 session获取 KEY：Const.SESSION_SECURITY_CODE
     * 开始时间 也存在session中
     * @author Ajie
     * @date 2019/10/31 0031
     * @param email -收件人
     * @param validity -有效期/分钟
     * @param code -验证码
     * @return
     */
    public static void setEmail(String email, String validity, String code) throws Exception {
        // 发送邮箱
        sendEmail(MAIL_FROM,email,
                MAIL_SUBJECT_VERIFY_CODE,
                prettyLayout(MAIL_ORGANIZATION,MAIL_WEBSITE,
                        MAIL_ORGANIZATION_LOGO,
                        prettyQrCodeLayout(validity,code)),
                MAIL_FROM_PASSWORD);
    ...
    public static void main(String[] args) throws Exception{
        ...
        sendEmail(MAIL_FROM,email,
                ...
                prettyLayout(MAIL_ORGANIZATION,MAIL_WEBSITE,
                        MAIL_ORGANIZATION_LOGO,
                        prettyQrCodeLayout("5分钟",RandomCodeUtil.getInvitaCode(6,0))),
                MAIL_FROM_PASSWORD);
...
//        session.setDebug(true); 去掉打印
        MimeMessage message = new MimeMessage(session);
        //设置邮件的头
        message.setFrom(new InternetAddress(  from  )); //谁发送的
        message.setRecipients(Message.RecipientType.TO,    to   );//发送给谁
        message.setRecipients(Message.RecipientType.CC, InternetAddress.parse(from)); // 抄送一份给自己
...
//        message.setText("<h1>hello</h1>");//纯文本
     ...
     * @param principalImage 主页logo 200 x 200 拉伸宽度为 (超链接形式)
     * @param business 业务
    ...
    private static String prettyLayout(String websiteName,String principal,String principalImage,String business,String...qrCode){
        return mailHead(websiteName,principal,principalImage,"200","200")+
                business +
