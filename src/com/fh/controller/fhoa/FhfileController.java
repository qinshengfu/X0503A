package com.fh.controller.fhoa;
...
import com.fh.util.FileDownloadUtil;
import com.fh.util.FileUtil;
import com.fh.util.PageData;
import com.fh.util.Jurisdiction;
import com.fh.util.PathUtil;
import com.fh.util.Tools;
import com.fh.service.fhoa.FhfileManager;
...
public class FhfileController extends BaseController {
	...
	public Object uploadImage(HttpServletRequest request) {
		...
		if (cmr.isMultipart(request)) {
			...
			while (files.hasNext()) {
				...
				mFile.getName();
				if (mFile != null) {
					path = FileUtil.saveFile(mFile, request, "release/lbt");
					//path = path.substring(path.indexOf("release/token"), path.length());
				}
			}
		}
	...
	public void download(HttpServletResponse response)throws Exception{
		...
		FileDownloadUtil.fileDownload(response, PathUtil.getClasspath() + Const.FILEPATHFILEOA + fileName, pd.getString("NAME")+fileName.substring(19, fileName.length()));
	}
